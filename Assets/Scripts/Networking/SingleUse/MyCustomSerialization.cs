...
public class MyCustomSerialization
{
    ...
    public static byte[] Serialize(object obj)
    {
        ...
        byte[] myNumberBtyes = BitConverter.GetBytes(data.MyNumber);
        if (BitConverter.IsLittleEndian)
            Array.Reverse(myNumberBtyes);
        ...
        return JoinBytes(myNumberBtyes, myStringBytes);
    }
    ...
    public static object Deserialize(byte[] bytes)
    {
        ...
        if (myStringBytes.Length > 0)
        {
            ...
            if (BitConverter.IsLittleEndian)
                Array.Reverse(myStringBytes);
            data.MyNumber = BitConverter.ToInt32(myStringBytes, 0);
        }
        else
        {
